...
def effectorTarget(parentNode, position=[0., 0., 200.0]):
    target = parentNode.createChild("Target")
    target.createObject("EulerImplicitSolver", firstOrder=True)
    target.createObject('VisualStyle', displayFlags='showVisualModels showInteractionForceFields showCollisionModels')
    target.createObject("CGLinearSolver")
    target.createObject("MechanicalObject", name="dofs", position=position, showObject=0, showObjectScale=0, drawMode=0, showColor=[1., 1., 1., 1.])
    target.createObject('SphereCollisionModel', radius='5')
...
class CosseratCable(SofaObject):
    ...
    def __addCables(self):
        ...
        for i in range(0, nbCables):           
            ...
            framesMO = mappedFrameNode.createObject('MechanicalObject', template='Rigid3d', name="FramesMO", position=VecFrame, showObject='0', showObjectScale='0', showIndices=0)
            ...
            slidingPointMO = slidingPoint.createObject('MechanicalObject', name="slidingPointMO"+str(i), position=cable_position, showObject="0", showIndices="0")
...
class Trunk(SofaObject):
    ...
    def addConstraintPoints(self, cstPoints,i,mappedPointsNode):
        ...
        inputFEMCable = trunkMappedPoints.createObject('MechanicalObject', name="pointsInFEM"+str(i), position=cstPoints, showObject="0", showIndices="0")
