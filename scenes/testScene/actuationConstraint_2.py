...
class DataComputationClass(CosseratActuation):
    ...
    def initGraph(self, node):
        ...
        self.vec_dy = [0.0, 0.0, -R_b/2.0]
        self.vec_dz = [R_b/2.0, -R_b/2.0, 0.0]
        self.vec_ddy = [R_b/(2.0*L), R_b/(2.0*L), 0.0]
        self.vec_ddz = [-R_b/(2.0*L), R_b/(2.0*L), 0.0]
...
def createScene(rootNode):
    ...
    rootNode.createObject(
        'RequiredPlugin', pluginName='SoftRobots SofaPython SofaSparseSolver CosseratPlugin BeamAdapter', printLog='0')
    rootNode.createObject(
        'VisualStyle', displayFlags='hideVisualModels showBehaviorModels showCollisionModels hideBoundingCollisionModels showForceFields hideInteractionForceFields hideWireframe')
    ...
    rigidBaseNode.createObject('RestShapeSpringsForceField', name='spring', stiffness="5000000",
                               angularStiffness="500000000", external_points="0", mstate="@RigidBaseMO", points="0", template="Rigid3d")
    ...
    BeamHookeLawForce = rateAngularDeformNode.createObject('CosseratInternalActuation', name="BeamHookeLawForce",  crossSectionShape='circular', length='25 25 25', radius='1.0',
                                                           youngModulus='2.7e4', distance0=_distance, distance1=_distance, tension=_tension)
    ...
    CylinderCollision.createObject('CylinderGridTopology', name="loader",
                                   nx="8", ny="8", nz="20", length="75", radius="1", axis="1 0 0")
