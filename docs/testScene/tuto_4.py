...
def createScene(root_node):
    ...
    const_force_node = cosserat_frames.addObject('ConstantForceField', name='constForce', showArrowSize=1.e-8, indices=geoParams.nbFrames, 
                                                 forces=force_null)
    
    # The endffectors is used only when force_type is 3
    ...
    controller_state = tip_controller.addObject('MechanicalObject', template='Rigid3d', name="controlEndEffector",
                            showObjectScale=0.3, position=[geoParams.beamLength, 0, 0, 0, 0, 0, 1], showObject=True)

    cosserat_frames.addObject('RestShapeSpringsForceField', name='spring',
                        stiffness=1e8, angularStiffness=1e8, external_points=0,
                        external_rest_shape=controller_state.getLinkPath(), points=geoParams.nbFrames, template="Rigid3d")
