...
bool init() {
  MessageDispatcher::addHandler(
      &MainPerComponentLoggingMessageHandler::getInstance());
  return true;
}
...
static struct Cosserat_Scene_tests : public PythonTestExtractor {
  Cosserat_Scene_tests() {
    std::string executable_directory =
        sofa::helper::Utils::getExecutableDirectory();
    if (sofa::helper::system::FileSystem::exists(
            "./Cosserat.Scene.Tests/Components")) {
      addTestDirectory("./Cosserat.Scene.Tests/Components", "Cosserat_Scene_");
    } else {
      addTestDirectory(executable_directory +
                           "/Cosserat.Scene.Tests.d/Components",
                       "Cosserat_Scene_");
    }
  }
} python_tests;
...
INSTANTIATE_TEST_SUITE_P(Cosserat, Cosserat,
