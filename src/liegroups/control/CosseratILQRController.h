...
public:
    ...
    Result optimize(const Trajectory& reference, const std::vector<Vector6>& initial_strains) {
        ...
        for (int iter = 0; iter < m_config.max_iterations; ++iter) {
            ...
            if (m_config.verbose) {
                std::cout << "iLQR Iteration " << iter + 1 
                          << ": Cost = " << cost_new
...
private:
     ...
     */
    std::pair<std::vector<SE3Type>, Scalar> forwardPass(
        const std::vector<Vector6>& u,
     ...
     */
    std::vector<Matrix6> backwardPass(
        const std::vector<SE3Type>& x,
        const std::vector<Vector6>& u,
        const Trajectory& ref
    ) const {
        const int T = static_cast<int>(u.size());
        ...
        for (int k = T - 1; k >= 0; --k) {
            ...
            Vector6 k = -Q_uu.ldlt().solve(Q_u);
            
            // Update value function
            V_x = Q_x + K[k].transpose() * Q_uu * k + Q_ux.transpose() * k + K[k].transpose() * Q_u;
     ...
     */
    std::pair<Matrix6, Matrix6> linearizeDynamics(
        const SE3Type& x,
        const Vector6& u,
        Scalar L
    ) const {
        ...
        auto [J_g1, J_g2] = SE3Type::composeJacobians(x, exp_u);
     ...
     */
    std::tuple<std::vector<Vector6>, std::vector<SE3Type>, Scalar, Scalar> lineSearch(
        const std::vector<SE3Type>& x,
        const std::vector<Vector6>& u,
        const std::vector<Matrix6>& K,
        const Trajectory& ref,
        Scalar current_cost
    ) const {
        Scalar alpha = 1.0;
     ...
     */
    std::vector<Matrix6> computeFeedbackGains(
        const std::vector<SE3Type>& x,
        const std::vector<Vector6>& u,
        const Trajectory& ref
    ) const {
