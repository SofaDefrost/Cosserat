...
template <typename _Scalar> class SE23;
...
Eigen::Matrix<_Scalar, 6, 1>
operator/(const Eigen::Matrix<_Scalar, 6, 1> &point_vel,
          const SE23<_Scalar> &g) {
 ...
 */
template <typename _Scalar>
SE23<_Scalar> fromComponents(const typename SE23<_Scalar>::Vector3 &position,
...
template <typename _Scalar>
SE23<_Scalar>
fromPositionEulerVelocity(const typename SE23<_Scalar>::Vector3 &position,
                          const _Scalar &roll, const _Scalar &pitch,
                          const _Scalar &yaw,
                          const typename SE23<_Scalar>::Vector3 &velocity) {
  return SE23<_Scalar>(fromPositionEulerZYX(position, roll, pitch, yaw),
                       velocity);
}
 ...
 */
template <typename _Scalar>
typename SE23<_Scalar>::Vector
toPositionEulerVelocity(const SE23<_Scalar> &transform) {
 ...
 */
template <typename _Scalar>
SE23<_Scalar> interpolate(const SE23<_Scalar> &from, const SE23<_Scalar> &to,
                          const _Scalar &t) {
 ...
 */
template <typename _Scalar>
Eigen::Matrix<_Scalar, 5, 5>
dualMatrix(const typename SE23<_Scalar>::TangentVector &xi) {
 ...
 */
template <typename _Scalar>
typename SE23<_Scalar>::TangentVector
SE23<_Scalar>::BCH(const TangentVector &X, const TangentVector &Y) {
  // Extract components
  ...
  result.template segment<3>(6) =
      a1 + a2 + _Scalar(0.5) * (w1_cross_a2 - a1_cross_w2);
