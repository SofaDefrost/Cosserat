...
public:
   ...
   */
  static Scalar SE3Distance(const Matrix4 &T1, const Matrix4 &T2,
                           Scalar w_rot = Scalar(1), Scalar w_trans = Scalar(1)) {
    // Rotation distance
   ...
   */
  static std::vector<Matrix4> generateSE3Trajectory(const std::vector<Matrix4> &waypoints,
                                                   int num_points = 10) {
    if (waypoints.size() < 2) {
      return waypoints;
    }
