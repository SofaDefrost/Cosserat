...
// Benchmarks for Lie group operations
...
public:
	Vector3 randomVector() { return Vector3(vec_dist(gen), vec_dist(gen), vec_dist(gen)); }
	...
	double randomAngle() { return angle_dist(gen); }
};
...
static void BM_SE3_Operations(benchmark::State &state) {
	RandomGenerator rng;
	SE3<double> transform(SO3<double>(rng.randomAngle(), rng.randomUnitVector()), rng.randomVector());
	Vector3 point = rng.randomVector();
...
static void BM_SE23_Operations(benchmark::State &state) {
	RandomGenerator rng;
	SE23<double> extended_pose(SE3<double>(SO3<double>(rng.randomAngle(), rng.randomUnitVector()), rng.randomVector()),
							   rng.randomVector());
	Vector3 point = rng.randomVector();
...
static void BM_Bundle_Operations(benchmark::State &state) {
	...
	PoseVel bundle(SE3<double>(SO3<double>(rng.randomAngle(), rng.randomUnitVector()), rng.randomVector()),
...
static void BM_CosseratRod_Operations(benchmark::State &state) {
	...
	for (int i = 0; i < num_segments; ++i) {
		segments.push_back(
				RodSegment(SE3<double>(SO3<double>(rng.randomAngle(), rng.randomUnitVector()), rng.randomVector()),
						   RealSpace<double, 3>(rng.randomVector())));
	}
...
BENCHMARK(BM_CosseratRod_Operations)->RangeMultiplier(2)->Range(8, 128);
...
static void BM_ExpMap_Operations(benchmark::State &state) {
	...
	for (auto _: state) {
		...
		auto transform = SE3<double>().exp((Eigen::Matrix<double, 6, 1>() << v, omega).finished());
...
static void BM_Interpolation_Operations(benchmark::State &state) {
	...
	SE3<double> T1(SO3<double>(rng.randomAngle(), rng.randomUnitVector()), rng.randomVector());
	SE3<double> T2(SO3<double>(rng.randomAngle(), rng.randomUnitVector()), rng.randomVector());
...
BENCHMARK(BM_Interpolation_Operations)->RangeMultiplier(2)->Range(8, 128);
