...
static void BM_JacobianComputation(benchmark::State &state) {
	ConcreteStrain2FramesCosseratMapping::TangentVector strain;
	strain << 0.1, 0.2, 0.3, 1.0, 0.1, 0.0;
	double curv_abs = 1.0;

	ConcreteStrain2FramesCosseratMapping::AdjointMatrix result;

	for (auto _: state) {
		ConcreteStrain2FramesCosseratMapping::computeTangExpImplementation(curv_abs, strain, result);
	}
}
