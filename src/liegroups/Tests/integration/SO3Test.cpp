...
namespace sofa::component::cosserat::liegroups::testing {
	...
	protected:
		...
		using AdjointMatrix = typename T::AdjointMatrix;
		using Quaternion = typename T::Quaternion;
	...
	TYPED_TEST(SO3Test, Constructors) {
		...
		typename TestFixture::Matrix m = g2.matrix();
		SO3 g4(m);
	 ...
	 * Verifies that applying `exp` to a Lie algebra element and then `log` to the resulting group element returns the
	 * original Lie algebra element.
	 */
	...
	TYPED_TEST(SO3Test, Action) {
		...
		SO3 g(Scalar(M_PI * 2.5), Vector::UnitZ()); // Angle > 2*PI, around Z
	...
	TYPED_TEST(SO3Test, Adjoint) {
		...
		SO3 g(Scalar(M_PI / 4.0), Eigen::Vector3d::UnitX());
		TangentVector omega;
		omega << 0.1, 0.2, 0.3;
