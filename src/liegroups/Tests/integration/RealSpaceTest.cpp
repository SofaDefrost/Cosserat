...
namespace sofa::component::cosserat::liegroups::testing {
	...
	TYPED_TEST(RealSpaceTest, Constructors) {
		...
		RealSpace g2(v);
		EXPECT_TRUE(g2.computeLog().isApprox(v, this->eps));
	}
	...
	TYPED_TEST(RealSpaceTest, Identity) {
		...
		EXPECT_TRUE(identity.computeLog().isApprox(Vector::Zero(), this->eps));
	}

	/**
	 * @brief Tests the inverse operation of the RealSpace group.
	 * Verifies that the inverse of a vector is its negation, and that composing a vector with its inverse results in
	 * the identity element (zero vector).
	 */
	TYPED_TEST(RealSpaceTest, Inverse) {
		...
		EXPECT_TRUE(inv_g.computeLog().isApprox(-v, this->eps));
	 ...
	 * returns the original Lie algebra element. For RealSpace, both `exp` and `log` are identity functions.
	 */
	 ...
	 * Verifies that a valid RealSpace element is correctly identified as valid, and an invalid one (e.g., containing
	 * NaN) is identified as invalid.
	 */
	 ...
	 * Verifies that `computeNormalize()` does not alter the RealSpace element, as no normalization is needed for
	 * RealSpace.
	 */
	TYPED_TEST(RealSpaceTest, Normalize) {
		...
		RealSpace g(v);
		g.computeNormalize();
		EXPECT_TRUE(g.computeLog().isApprox(v, this->eps)); // RealSpace normalize does nothing
	}

	/**
	 * @brief Tests the squared distance calculation for RealSpace elements.
	 * Verifies that the squared distance is correctly computed as the squared Euclidean norm of the difference between
	...
	TYPED_TEST(RealSpaceTest, Interpolate) {
		...
		EXPECT_TRUE(interpolated.computeLog().isApprox((v1 + v2) / 2.0, this->eps));
