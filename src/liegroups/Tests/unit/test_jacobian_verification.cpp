...
protected:
	...
	AdjointMatrix computeNumericalJacobian(double curv_abs, const TangentVector &strain, double eps = 1e-7) {
...
TEST_F(JacobianVerificationTest, ZeroStrain) {
	...
	MappingType::computeTangExpImplementation(curv_abs, strain, adjoint, tang_adjoint);

	AdjointMatrix expected = curv_abs * AdjointMatrix::Identity();

	EXPECT_TRUE(tang_adjoint.isApprox(expected, tolerance_)) << "Zero strain Jacobian should be curv_abs * I\n"
															 << "Expected:\n"
															 << expected << "\n"
															 << "Got:\n"
															 << tang_adjoint;
...
TEST_F(JacobianVerificationTest, NumericalAccuracy) {
	...
	MappingType::computeTangExpImplementation(curv_abs, strain, adjoint, analytical_jac);
...
TEST_F(JacobianVerificationTest, Consistency) {
	...
	MappingType::computeTangExpImplementation(curv_abs, strain, adjoint, tang_adjoint1);

	MappingType::computeTangExpImplementation(curv_abs, strain, adjoint, tang_adjoint2);

	// Results should be identical
	EXPECT_TRUE(tang_adjoint1.isApprox(tang_adjoint2, 1e-15)) << "Jacobian computation is not deterministic";
}
...
TEST_F(JacobianVerificationTest, ScalingProperties) {
	...
	MappingType::computeTangExpImplementation(curv_abs1, strain, adjoint1, tang_adjoint1);

	MappingType::computeTangExpImplementation(curv_abs2, strain, adjoint2, tang_adjoint2);
