...
namespace sofa::component::cosserat::liegroups {
	...
	public:
		...
		SE3(const SO3Type &rotation, const Vector3 &translation) : m_rotation(rotation), m_translation(translation) {}
		...
		explicit SE3(const Matrix4 &matrix) :
			m_rotation(matrix.template block<3, 3>(0, 0)), m_translation(matrix.template block<3, 1>(0, 3)) {}
		...
		SE3 compose(const SE3 &other) const {
			return SE3(m_rotation * other.m_rotation, m_rotation.act(other.m_translation) + m_translation);
		}
		...
		TangentVector computeTangent(const ActionVector &point) const {
			TangentVector tangent;
			tangent.template head<3>() = m_rotation.inverse().act(point - m_translation);
			tangent.template tail<3>() = m_rotation.inverse().log();
			return tangent;
		}
		...
		AdjointMatrix ad(const ActionVector &point) {
			AdjointMatrix Ad = AdjointMatrix::Zero();
			const Vector3 rho = point.template head<3>();
			const Vector3 omega = point.template tail<3>();
			const Matrix3 R = m_rotation.matrix();
		...
		AdjointMatrix ad(const TangentVector &v, const TangentVector &w) {
			AdjointMatrix Ad = AdjointMatrix::Zero();
			const Vector3 omega_v = v.template tail<3>();
			const Vector3 rho_v = v.template head<3>();
			const Vector3 omega_w = w.template tail<3>();
			const Vector3 rho_w = w.template head<3>();
			const Matrix3 R = m_rotation.matrix();
		...
		AdjointMatrix buildProjectionMatrix(const Matrix3 & rotation) const {
			...
			proj_matrix.template block<3, 3>(3, 0) = rotation.matrix();
	...
	private:
		 ...
		 */
		static SE3 expCosseratGeneral(const Vector3 &rho, const Vector3 &phi, const Scalar &phi_norm,
									  const Scalar &length) noexcept {
			...
			const Matrix4 g_x = Matrix4::Identity() + length * xi_hat + alpha * xi_hat2 + beta * xi_hat3;
