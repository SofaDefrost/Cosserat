...
namespace sofa::component::cosserat::liegroups {
	...
	public:
		 ...
		 */
		explicit SO2(const Scalar &angle) : m_angle(Types<_Scalar>::normalizeAngle(angle)) { updateComplex(); }
		...
		/**
		 * @brief Composes two SO(2) elements (group multiplication).
		 * @param other The other SO(2) element to compose with.
		 * @return The composed SO(2) element (sum of angles).
		 */
		SO2<Scalar> compose(const SO2 &other) const { return SO2(m_angle + other.m_angle); }
		 ...
		 */
		static SO2<Scalar> computeExp(const TangentVector &algebra_element) { return SO2(algebra_element[0]); }
		/**
		...
		std::ostream &print(std::ostream &os) const {
			os << "SO2(angle=" << m_angle << " rad, " << (m_angle * Scalar(180) / M_PI) << " deg)";
			return os;
		}
		 ...
		 */
		static Matrix computeHat(const TangentVector &omega) {
		 ...
		 */
		static TangentVector computeVee(const Matrix &matrix) {
			TangentVector result;
			result[0] = matrix(1, 0);
			return result;
		}
	...
	SO2<Scalar> SO2FromDegrees(const Scalar &degrees) {
	 ...
	 */
	template<typename Scalar>
	Scalar SO2ToDegrees(const SO2<Scalar> &rotation) {
