...
template <typename... Groups> struct TotalDimension;

template <> struct TotalDimension<> {
...
template <typename... Groups> struct TotalActionDimension;

template <> struct TotalActionDimension<> {
  static constexpr int value = 0;
};
...
// type
template <typename FirstScalar, typename... Groups> struct AllAreLieGroups;
...
template <int Index, typename... Groups> struct OffsetAt;

template <int Index> struct OffsetAt<Index> {
  static constexpr int value = 0;
};
...
struct OffsetAt<Index, Group, Rest...> {
  static constexpr int value =
      (Index == 0) ? 0 : Group::Dim + OffsetAt<Index - 1, Rest...>::value;
};

// Runtime offset computation for action dimensions
template <typename... Groups> class ActionOffsets {
public:
...
private:
  ...
  void computeOffsets(const std::tuple<Groups...> &groups,
...
public:
   ...
   */
  explicit Bundle(const TangentVector &algebra_element) : Bundle() {
   ...
   */
  template <int N>
  Eigen::Matrix<Scalar, Eigen::Dynamic, N>
  act(const Eigen::Matrix<Scalar, Eigen::Dynamic, N> &points) const {
   ...
   */
  template <std::size_t I> void set(const GroupType<I> &group) {
...
private:
  ...
  detail::ActionOffsets<Groups...>
      m_action_offsets; ///< Cached action dimension offsets
  ...
  template <std::size_t... Is>
  Bundle exp_impl(const TangentVector &algebra_element,
                  std::index_sequence<Is...>) const {
    return Bundle((GroupType<Is>().exp(
        algebra_element.template segment<GroupType<Is>::Dim>(
            AlgebraOffset<Is>)))...);
  }
  ...
  AdjointMatrix adjoint_impl(std::index_sequence<Is...>) const {
    ...
    ((result.template block<GroupType<Is>::Dim, GroupType<Is>::Dim>(
          AlgebraOffset<Is>, AlgebraOffset<Is>) =
  ...
  bool isApprox_impl(const Bundle &other, const Scalar &eps,
                     std::index_sequence<Is...>) const {
    return (
        std::get<Is>(m_groups).isApprox(std::get<Is>(other.m_groups), eps) &&
        ...);
  }
...
// Convenience aliases
template <typename... Groups> using Bundlef = Bundle<Groups...>;

template <typename... Groups> using Bundled = Bundle<Groups...>;
...
void moduleAddSO2(py::module &m) {
  ...
  py::class_<SO2<double>>(m, "SO2")
      ...
      .def("act",
           static_cast<typename SO2<double>::Vector (SO2<double>::*)(
...
void moduleAddSO3(py::module &m) {
  ...
  py::class_<SO3<double>>(m, "SO3")
      ...
      .def("act", static_cast<typename SO3<double>::Vector (SO3<double>::*)(
                      const typename SO3<double>::Vector &) const>(
                      &SO3<double>::computeAction));
}
...
void moduleAddSE2(py::module &m) {
  ...
  py::class_<SE2<double>>(m, "SE2")
      ...
      .def("rotation", static_cast<const SO2<double> &(SE2<double>::*)() const>(
                           &SE2<double>::rotation))
      .def("translation",
           static_cast<const typename SE2<double>::Vector2 &(SE2<double>::*)()
                           const>(&SE2<double>::translation))
      ...
      .def("act", static_cast<typename SE2<double>::Vector2 (SE2<double>::*)(
                      const typename SE2<double>::Vector2 &) const>(
                      &SE2<double>::act));
}
...
void moduleAddSE3(py::module &m) {
  ...
  py::class_<SE3<double>>(m, "SE3")
      ...
      .def("rotation", static_cast<const SO3<double> &(SE3<double>::*)() const>(
                           &SE3<double>::rotation))
      .def("translation",
           static_cast<const typename SE3<double>::Vector3 &(SE3<double>::*)()
                           const>(&SE3<double>::translation))
      ...
      .def(
          "co_adjoint",
          [](const SE3<double> &self) { return self.adjoint().transpose(); },
      ...
      .def("act",
           static_cast<typename SE3<double>::Vector3 (SE3<double>::*)(
