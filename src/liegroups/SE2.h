...
namespace sofa::component::cosserat::liegroups {
	...
	class SE2 : public LieGroupBase<SE2<_Scalar>, _Scalar, 3, 3, 2> {
		 ...
		 * @param translation The 2D translation vector.
		 */
		SE2(const Scalar &angle, const Vector2 &translation) : m_rotation(angle), m_translation(translation) {}
		 ...
		 * element.
		 */
		explicit SE2(const TangentVector &tangent_vector) { *this = exp(tangent_vector); }
		...
		static SE2 computeExp(const TangentVector &algebra_element) {
			...
			Matrix2 V;
			V << sin_over_theta, -one_minus_cos_over_theta, one_minus_cos_over_theta, sin_over_theta;
		...
		TangentVector computeLog() const {
			...
			Matrix2 V_inv;
			V_inv << half_theta * cot_half, -half_theta, half_theta, half_theta * cot_half;
		...
		std::ostream &print(std::ostream &os) const {
			os << "SE2(angle=" << m_rotation.angle() << ", translation=(" << m_translation.transpose() << "))";
			return os;
		}
		 ...
		 * otherwise.
		 */
		bool computeIsValid() const { return m_rotation.computeIsValid() && m_translation.allFinite(); }
		 ...
		 */
		static SE2 fromTranslation(const Vector2 &translation) { return SE2(SO2<Scalar>(), translation); }
		 ...
		 */
		static SE2 fromRotation(const Scalar &angle) { return SE2(SO2<Scalar>(angle), Vector2::Zero()); }
		 ...
		 * @return An SE2 object with the given rotation and zero translation.
		 */
		static SE2 fromRotation(const SO2<Scalar> &rotation) { return SE2(rotation, Vector2::Zero()); }
		...
		friend std::ostream &operator<<(std::ostream &os, const SE2 &se2) {
			os << "SE2(angle=" << se2.m_rotation.angle() << ", translation=(" << se2.m_translation.transpose() << "))";
			return os;
		}
	...
	private:
		 ...
		 * otherwise.
		 */
		static bool isValidTransformationMatrix(const Matrix3 &matrix, const Scalar &eps = Types<Scalar>::epsilon()) {
			// Check bottom row is [0, 0, 1]
			if (!matrix.template block<1, 2>(2, 0).isZero(eps) || std::abs(matrix(2, 2) - Scalar(1)) > eps) {
				return false;
			}
