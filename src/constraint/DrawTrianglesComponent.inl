...
void DrawTrianglesComponent::draw(const core::visual::VisualParams* vparams)
{
    ...
    if (vparams->displayFlags().getShowWireFrame()) {
        ...
        for(auto it = d_vecTriangles.getValue().begin(); it !=d_vecTriangles.getValue().end() ; ++it, ++i)
        {
            ...
            glColor4fv(ca); helper::gl::glVertexT(pa);
            glColor4fv(cb); helper::gl::glVertexT(pb);
            glColor4fv(cc); helper::gl::glVertexT(pc);
        }
        glEnd();
        ...
        for(auto it = d_vecTetra.getValue().begin(); it !=d_vecTetra.getValue().end() ; ++it, ++i)
        {
            ...
            Index c = (*it)[2];
            Index d = (*it)[3];
            nodeColors[0] = evalColor(vMN[a]);nodeColors[0][3] = d_transparency.getValue();
            nodeColors[1] = evalColor(vMN[b]);nodeColors[1][3] = d_transparency.getValue();
            nodeColors[2] = evalColor(vMN[c]);nodeColors[2][3] = d_transparency.getValue();
            nodeColors[3] = evalColor(vMN[d]);nodeColors[3][3] = d_transparency.getValue();
            float* ca = &nodeColors[0][0];
            float* cb = &nodeColors[1][0];
            float* cc = &nodeColors[2][0];
            ...
            glColor4fv(ca); helper::gl::glVertexT(pa);
            glColor4fv(cb); helper::gl::glVertexT(pb);
            glColor4fv(cd); helper::gl::glVertexT(pd);
