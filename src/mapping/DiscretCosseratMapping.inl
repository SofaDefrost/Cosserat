...
void DiscretCosseratMapping<TIn1, TIn2, TOut>:: applyJ(
    ...
    for (size_t i = 0 ; i < sz; i++) {
        ...
        Vec6 temp = Adjoint * (this->m_nodesVelocityVectors[this->m_indicesVectors[i]-1] + this->m_framesTangExpVectors[i] * Xi_dot ); // eta
...
void DiscretCosseratMapping<TIn1, TIn2, TOut>::draw(const core::visual::VisualParams* vparams)
{
    if (!vparams->displayFlags().getShowMappings())
        ...
        if(!this->d_debug.getValue()) return;
    for (unsigned int i = 0;i < this->m_vecTransform.size(); i++) {

        defaulttype::Quat q = this->m_vecTransform[i].getOrientation();
        q.normalize();

        defaulttype::Vector3 P1, x,y,z;
        P1 = this->m_vecTransform[i].getCenter();
