...
namespace Cosserat::mapping {
	...
	void DiscreteCosseratMapping<Vec6Types, Rigid3Types, Rigid3Types>::applyJ(
		...
		msg_info("DiscreteCosseratMapping") << " ########## ApplyJ (vel) function: Rig ########";
		...
		const auto inDeform =
				sofa::helper::getReadAccessor(*m_strain_state->read(sofa::core::vec_id::read_access::position));
		...
		const sofa::VecCoord_t<In2> &xfrom2Data =
				sofa::helper::getReadAccessor(*m_rigid_base->read(sofa::core::vec_id::read_access::position));
		...
		msg_info("DiscreteCosseratMapping") << "Base local Velocity :" << baseLocalVelocity;
		...
		for (unsigned int i = 1; i < curv_abs_section.size(); i++) {
			...
			// @todo Why compute adjoint are on exponential ? and not on frames ?
			TangentTransform Adjoint;
			this->compute_matrix_Adjoint(Trans, Adjoint);

			Vec6 node_Xi_dot;
			for (unsigned int u = 0; u < 6; u++)
				node_Xi_dot(i) = in1_vel[i - 1][u];

			Vec6 eta_node_i = Adjoint * (m_nodes_velocity_vectors[i - 1] + m_nodes_tang_exp_vectors[i] * node_Xi_dot);
			m_nodes_velocity_vectors.push_back(eta_node_i);
			// msg_info("DiscreteCosseratMapping") << "Node velocity : "<< i << " = " << eta_node_i;
		}
		msg_info("DiscreteCosseratMapping") << "Node Velocities : " << m_nodes_velocity_vectors;
		const sofa::VecCoord_t<Out> &out =
				sofa::helper::getReadAccessor(*m_global_frames->read(sofa::core::vec_id::read_access::position));
		...
		for (unsigned int i = 0; i < sz; i++) {
			...
			// @todo Why compute adjoint are on exponential ? and not on frames ?
			this->compute_matrix_Adjoint(Trans, Adjoint);
			Vec6 frame_Xi_dot = in1_vel[m_indices_vectors[i] - 1];
			...
			// msg_info("DiscreteCosseratMapping") << "Frame velocity : "<< i << " = " << out_vel[i];
		}
		// msg_info("DiscreteCosseratMapping")<< "Frame Output Velocities : "<< out_vel;
		std::cout << " DiscreteCosseratMapping::applyJ completed out vel frames : " << out_vel << std::endl;

		std::cout << "m index input : " << m_index_input << std::endl;
		m_index_input = 0;
	}
	...
	void DiscreteCosseratMapping<Vec6Types, Rigid3Types, Rigid3Types>::applyJT(
		...
		msg_info("DiscreteCosseratMapping") << " ########## ApplyJT force R Function ########";
		...
		for (unsigned int var = 0; var < in.size(); ++var) {
			...
			for (unsigned j = 0; j < 6; j++)
				vec[j] = in[var][j];
			// std::cout << " Input force at var "<< var << " is : "<< vec << std::endl;
		...
		std::cout << " The size of indices vector is : " << sz << std::endl;
		...
		for (auto s = sz; s--;) {
			TangentTransform coAdjoint_gx_frame;

			this->compute_matrix_CoAdjoint(m_frames_exponential_se3_vectors[s], coAdjoint_gx_frame);
			Vec6 vec_force_frame = coAdjoint_gx_frame * local_F_Vec[s];
			Mat6x6 temp =
					m_frames_tang_exp_vectors[s]; // m_framesTangExpVectors[s] computed in applyJ (here we transpose)
			temp.transpose();
			Vec6 f = matB_trans * temp * vec_force_frame;

			if (index != m_indices_vectors[s]) {
				index--;
				// bring F_tot to the reference of the new beam
				TangentTransform coAdjoint_gx_node;
				this->compute_matrix_CoAdjoint(m_nodes_exponential_se3_vectors[index], coAdjoint_gx_node);
				F_tot = coAdjoint_gx_node * F_tot;
				Mat6x6 temp_mat = m_nodes_tang_exp_vectors[index];
				temp_mat.transpose();
			...
			// msg_info("DiscreteCosseratMapping") << "f at s ="<< s <<" and index"<< index <<  " is : "<< f;

			// compute F_tot
			F_tot += vec_force_frame;
		...
		msg_info("DiscreteCosseratMapping") << "Node forces " << out1 << msgendl << "\nbase Force: " << out2[baseIndex];
	}
	...
	void DiscreteCosseratMapping<Vec6Types, Rigid3Types, Rigid3Types>::applyJT(
		...
		msg_info("DiscreteCosseratMapping") << " ########## ApplyJT constraint R Function ########";

		// We need only one input in models and input Root model (if present)
		sofa::MatrixDeriv_t<In1> &out1 = sofa::helper::getWriteAccessor(
				*dataMatOut1Const[0]); // constraints on the strain space (reduced coordinate)
		sofa::MatrixDeriv_t<In2> &out2 =
				sofa::helper::getWriteAccessor(*dataMatOut2Const[0]); // constraints on the reference frame (base frame)
		const sofa::MatrixDeriv_t<Out> &in =
				dataMatInConst[0]->getValue(); // input constraints defined on the mapped frames
		...
		for (sofa::MatrixDeriv_t<Out>::RowConstIterator rowIt = in.begin(); rowIt != rowItEnd; ++rowIt) {
			msg_info_when(doPrintLog) << "************* Apply JT (MatrixDeriv) iteration on line " << rowIt.index()
									  << "*************  ";
			...
			if (colIt == colItEnd) {
				msg_info_when(doPrintLog) << "no column for this constraint";
				continue;
			}
			...
			while (colIt != colItEnd) {
				...
				TangentTransform frame_projector = (this->buildProjector(_T));
				frame_projector.transpose();

				TangentTransform coAdjoint_gx_frame;
				this->compute_matrix_CoAdjoint(
						m_frames_exponential_se3_vectors[childIndex],
						coAdjoint_gx_frame); // m_framesExponentialSE3Vectors[s] computed in apply
				auto tang_exp_frame = m_frames_tang_exp_vectors[childIndex]; // m_framesTangExpVectors[s] computed in
																			 // applyJ (here we transpose)
				tang_exp_frame.transpose();

				Vec6 local_force = coAdjoint_gx_frame * frame_projector *
								   valueConst; // constraint direction in local frame of the beam.

				Vec6 f = matB_trans * tang_exp_frame * local_force; // constraint direction in the strain space.

				o1.addCol(indexBeam - 1, f);
				std::tuple<int, Vec6> node_force = std::make_tuple(indexBeam, local_force);
			...
			if (doPrintLog) {
				...
				for (size_t i = 0; i < NodesInvolvedCompressed.size(); i++)
					tmp << "index :" << get<0>(NodesInvolvedCompressed[i])
						<< " force :" << get<1>(NodesInvolvedCompressed[i]) << msgendl;
				msg_info() << tmp.str();
			}
			...
			for (unsigned n = 0; n < NodesInvolvedCompressed.size(); n++) {
				...
				int i = numNode;
				Vec6 cumulative_force = std::get<1>(test);

				while (i > 0) {
					// cumulate on beam frame
					Mat6x6 co_adjoint_gx_node;
					this->compute_matrix_CoAdjoint(m_nodes_exponential_se3_vectors[i - 1], co_adjoint_gx_node);
					cumulative_force = co_adjoint_gx_node * cumulative_force;
					// transfer to strain space (local coordinates)
					Mat6x6 node_tang_exp = m_nodes_tang_exp_vectors[i - 1];
					node_tang_exp.transpose();
					Vec6 temp_f = matB_trans * node_tang_exp * cumulative_force;
				...
				Vec6 base_force = M * cumulative_force;
	...
	template class SOFA_COSSERAT_API DiscreteCosseratMapping<
	...
	void registerDiscreteCosseratMapping(sofa::core::ObjectFactory *factory) {
		factory->registerObjects(
				sofa::core::ObjectRegistrationData(
						"This component facilitates the creation of Cosserat Cables in SOFA simulations. It takes two "
						"mechanical"
						...
						.add<mapping::DiscreteCosseratMapping<sofa::defaulttype::Vec3Types,
															  sofa::defaulttype::Rigid3Types,
															  sofa::defaulttype::Rigid3Types>>(true)
						.add<mapping::DiscreteCosseratMapping<sofa::defaulttype::Vec6Types,
															  sofa::defaulttype::Rigid3Types,
															  sofa::defaulttype::Rigid3Types>>());
	}
} // namespace Cosserat
