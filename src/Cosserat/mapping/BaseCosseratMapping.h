...
namespace Cosserat::mapping {
	...
	namespace {
		...
		using SO3 = Mat3x3; ///< The "coordinate" in SE3
	...
	public:
		...
		// : dmarchal: it look like as if you should be working in a branch for making new feature and merge it when it
		// is ready.
	...
	public:
		...
		SE3 buildXiHat(const Vec3 &strain_i);
		SE3 buildXiHat(const Vec6 &strain_i);

		double computeTheta(const double &x, const Mat4x4 &gX);
		void printMatrix(const Mat6x6 R);

		static SO3 extract_tild_matrix(const Vec3 &u);
		static Mat3x3 extract_rotation_matrix(const Frame &frame);
	...
	protected:
		...
		void computeExponentialSE3(const double &sub_section_length, const Coord1 &k, Frame &frame_i);

		void compute_matrix_adj(const Vec6 &Xi, Mat6x6 &adjoint);
		void compute_matrix_coadj(const Vec6 &Xi, Mat6x6 &adjoint);

		void compute_matrix_Adjoint(const Frame &frame, TangentTransform &adjoint);
		void compute_matrix_CoAdjoint(const Frame &frame, Mat6x6 &coAdjoint);
		...
		//[[maybe_unused]] Vec6 computeETA(const Vec6 &baseEta, const vector<Deriv1> &k_dot, double abs_input);
		Mat4x4 computeLogarithm(const double &x, const Mat4x4 &gX);
	};

#if !defined(SOFA_COSSERAT_CPP_BaseCosseratMapping)
	extern template class SOFA_COSSERAT_API BaseCosseratMapping<
