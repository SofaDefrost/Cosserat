...
template <class TIn1, class TIn2, class TOut>
void BaseCosserat<TIn1, TIn2, TOut>::update_ExponentialSE3(
  ...
  for (size_t i = 0; i < sz; i++) {
    ...
    const Coord1 strain_n = inDeform[m_indicesVectors[i] -
                                     1]; // Cosserat reduce coordinates (strain)
    ...
    if (d_debug.getValue()) {
      msg_info("BaseCosserat:")
          << "_________________" << i << "_________________________";
      msg_info("BaseCosserat:")
          << "x :" << curv_abs_x << "; strain :" << strain_n;
      msg_info("BaseCosserat:")
  ...
  m_nodesExponentialSE3Vectors.push_back(
      Transform(type::Vec3(0.0, 0.0, 0.0),
                type::Quat(0., 0., 0., 1.))); // The first node.
  ...
  for (unsigned int j = 0; j < inDeform.size(); j++) {
    ...
    if (d_debug.getValue()) {
      msg_info("BaseCosserat:")
          << "_________________Beam Node Expo___________________" << j;
      msg_info("BaseCosserat:")
          << "Node m_framesExponentialSE3Vectors :" << g_X_node_j;
      msg_info("BaseCosserat:")
...
template <class TIn1, class TIn2, class TOut>
void BaseCosserat<TIn1, TIn2, TOut>::compute_adjointVec6(const Vec6 &eta,
                                                         Mat6x6 &adjoint) {
...
template <class TIn1, class TIn2, class TOut>
void BaseCosserat<TIn1, TIn2, TOut>::update_TangExpSE3(
    const In1VecCoord &inDeform) {
  ...
  for (unsigned int i = 0; i < sz; i++) {
    ...
    if (d_debug.getValue()) {
      printf("__________________________________________\n");
      std::cout << "x :" << curv_abs_x_i << "; k :" << strain_frame_i
                << std::endl;
      std::cout << "m_framesTangExpVectors :" << m_framesTangExpVectors[i]
                << std::endl;
    }
  }
...
template <class TIn1, class TIn2, class TOut>
void BaseCosserat<TIn1, TIn2, TOut>::compute_Tang_Exp(double &curv_abs_n,
                                                      const Coord1 &strain_i,
                                                      Mat6x6 &TgX) {

  SReal theta = type::Vec3(strain_i(0), strain_i(1), strain_i(2))
                    .norm(); // Sometimes this is computed over all strain
  Matrix3 tilde_k =
      getTildeMatrix(type::Vec3(strain_i(0), strain_i(1), strain_i(2)));
  /* Younes @23-11-27
  old version
...
template <class TIn1, class TIn2, class TOut>
[[maybe_unused]] type::Vec6
BaseCosserat<TIn1, TIn2, TOut>::compute_eta(const type::Vec6 &baseEta,
  ...
  helper::ReadAccessor<Data<type::vector<double>>> curv_abs_input =
      d_curv_abs_section;
  // helper::ReadAccessor<Data<helper::vector<double>>> curv_abs_output =
  // d_curv_abs_frames;
...
void BaseCosserat<TIn1, TIn2, TOut>::initialize() {
  ...
  if (d_debug.getValue()) {
    ...
    msg_info("BaseCosserat:")
        << "m_framesLengthVectors : " << m_framesLengthVectors;
    msg_info("BaseCosserat:")
        << "m_BeamLengthVectors : " << m_BeamLengthVectors;
  }
}
