...
using Vector6 = typename SE3Types::TangentVector;  // SE3 utilise TangentVector pour les vecteurs 6D
using Matrix3 = typename SE3Types::Matrix3;
using Matrix4 = typename SE3Types::Matrix4;
using AdjointMatrix = typename SE3Types::AdjointMatrix;
using JacobianMatrix = typename SE3Types::JacobianMatrix;
...
public:
    ...
    SectionInfo(double length, const Vector3& kappa, unsigned int i0, unsigned int i1,
                const SE3Types& transform = SE3Types::computeIdentity())
        : sec_length_(length), kappa_(kappa), index_0_(i0), index_1_(i1),
    ...
    void setIndices(unsigned int i0, unsigned int i1) {
        if (i0 >= i1) throw std::invalid_argument("index_0 must be less than index_1");
        index_0_ = i0;
        index_1_ = i1;
    }

    // Accesseurs pour la transformation SE3
    const SE3Types& getTransformation() const { return transformation_; }
    void setTransformation(const SE3Types& transform) {
        transformation_ = transform;
    ...
    void setTransformationFromMatrix(const Matrix4& matrix) {
        transformation_ = SE3Types(matrix);
    ...
    const AdjointMatrix& getCoAdjoint() const {
        if (!adjoint_computed_) {
            getAdjoint(); // Calcule les deux matrices
        }
        return coAdjoint_;
    }
     ...
     */
    SE3Types getLocalTransformation(double t) const {
        if (t < 0.0 || t > 1.0) {
        ...
        return transformation_ * SE3Types::computeExp(xi);
    }
    ...
    SectionInfo interpolate(const SectionInfo& other, double t) const {
        SE3Types interp_transform = transformation_.interpolate(other.transformation_, t);
    ...
    Vector3 transformPoint(const Vector3& point) const {
        return transformation_.computeAction(point);
    }
    ...
    SectionInfo compose(const SectionInfo& other) const {
        SE3Types composed_transform = transformation_.compose(other.transformation_);
...
protected:
    ...
    bool validateSectionProperties() const {
        for (size_t i = 0; i < m_sectionProperties.size(); ++i) {
            ...
            if (section.getIndex0() >= section.getIndex1()) {
                msg_warning() << "Section " << i << " has invalid indices: "
    ...
    bool checkContinuity(double eps = 1e-6) const {
        for (size_t i = 0; i < m_sectionProperties.size() - 1; ++i) {
            SE3Types end_transform = m_sectionProperties[i].getLocalTransformation(1.0);
            SE3Types start_transform = m_sectionProperties[i+1].getLocalTransformation(0.0);
     ...
     */
    std::vector<SE3Types> generateSmoothTrajectory(int num_points = 10) const {
        std::vector<SE3Types> trajectory;
...
protected:
    sofa::Data<std::vector<double>> d_curv_abs_section;
    sofa::Data<std::vector<double>> d_curv_abs_frames;
...
HookeSeratBaseMapping<sofa::defaulttype::Vec3Types, sofa::defaulttype::Rigid3Types,
                    sofa::defaulttype::Rigid3Types>;
extern template class SOFA_COSSERAT_API
HookeSeratBaseMapping<sofa::defaulttype::Vec6Types, sofa::defaulttype::Rigid3Types,
                    sofa::defaulttype::Rigid3Types>;
#endif
