...
class SE3 final : public LieGroupBase<SE3<Scalar>, Scalar,
                                      6> { // Use default values for _AlgebraDim
                                           // and _ActionDim
...
public:
  ...
  TangentVector log() const {
    ...
    if (theta < Types<Scalar>::epsilon()) {
      ...
      Matrix3 omega_hat = SO3<Scalar>::hat(omega);
      V_inv = Matrix3::Identity() - Scalar(0.5) * omega_hat +
   ...
   */
  static SE3 Identity() noexcept { return SE3(); }
  ...
  SE3 compose(const SE3 &other) const noexcept { return (*this) * other; }

  SE3 computeInverse() const { return inverse(); }

  static SE3 computeExp(const TangentVector &v) noexcept { return exp(v); }

  TangentVector computeLog() const { return log(); }

  AdjointMatrix computeAdjoint() const noexcept { return adjoint(); }
  ...
  computeAction(const typename Base::ActionVector &point) const noexcept {
    return act(point);
  }
  ...
  static TangentVector BCH(const TangentVector &X, const TangentVector &Y) {
    ...
    result.template head<3>() =
        v1 + v2 + Scalar(0.5) * (w1_cross_v2 - v1_cross_w2);
