...
template <typename _Scalar, int _Dim> class LieAlgebra;
...
struct InverseFailureException : public LieGroupException {
  InverseFailureException()
      : LieGroupException("Failed to compute inverse: singular element") {}
};

struct LogarithmException : public LieGroupException {
  LogarithmException()
      : LieGroupException("Logarithm undefined at this point") {}
};

struct NumericalInstabilityException : public LieGroupException {
  NumericalInstabilityException(const std::string &msg)
      : LieGroupException("Numerical instability: " + msg) {}
};
 ...
 * as _Dim)
 */
template <typename Derived, FloatingPoint _Scalar, int _Dim,
          int _AlgebraDim = _Dim, int _ActionDim = _Dim>
...
public:
   ...
   */
  [[nodiscard]] static constexpr Derived
  exp(const TangentVector &algebra_element) noexcept {
   ...
   */
  [[nodiscard]] AdjointMatrix adjoint() const noexcept {
   ...
   * @return The transformed point
   */
  [[nodiscard]] ActionVector
  operator()(const ActionVector &point) const noexcept {
    return act(point);
  }
   ...
   */
  [[nodiscard]] static constexpr Derived Identity() noexcept {
   ...
   */
  [[nodiscard]] Derived interpolate(const Derived &other,
   ...
   * @return Interpolated group element
   */
  [[nodiscard]] Derived lerp(const Derived &other,
                             const Scalar &t) const noexcept {
   ...
   * @return Interpolated group element
   */
  [[nodiscard]] Derived slerp(const Derived &other,
                              const Scalar &t) const noexcept;
   ...
   */
  [[nodiscard]] JacobianMatrix
  actionJacobian(const ActionVector &point) const noexcept;
   ...
   */
  [[nodiscard]] static TangentVector BCH(const TangentVector &v,
                                         const TangentVector &w, int order = 3);
   ...
   */
  [[nodiscard]] static constexpr int manifoldDim() noexcept {
    return AlgebraDim;
  }
   ...
   */
  [[nodiscard]] bool
  isNearIdentity(const Scalar &eps = Types::epsilon()) const noexcept {
   ...
   */
  friend std::ostream &operator<<(std::ostream &os, const LieGroupBase &g) {
    return g.derived().print(os);
  }
 ...
 */
template <typename T> struct is_lie_group : std::false_type {};

template <typename Derived, FloatingPoint Scalar, int Dim, int AlgDim,
          int ActDim>
struct is_lie_group<LieGroupBase<Derived, Scalar, Dim, AlgDim, ActDim>>
    : std::true_type {};
