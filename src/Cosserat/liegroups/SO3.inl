...
#ifndef SOFA_COMPONENT_COSSERAT_LIEGROUPS_SO3_INL
#define SOFA_COMPONENT_COSSERAT_LIEGROUPS_SO3_INL
...
template<typename _Scalar>
SO3<_Scalar>
SO3<_Scalar>::interpolate(const SO3& other, const Scalar& t) const noexcept {
...
SO3<_Scalar>::BCH(const TangentVector& v, const TangentVector& w, int order) {
    ...
    if (order >= 2) {
        ...
        if (order >= 3) {
            ...
            result += (vee(Vhat * hat(vw)) - vee(What * hat(vw))) * Scalar(1.0/12.0);
        }
    }
...
SO3<_Scalar>::dexp(const TangentVector& v) {
    ...
    return Matrix::Identity() + 
           (Scalar(1) - std::cos(theta)) / theta2 * V +
...
SO3<_Scalar>::dlog() const {
    ...
    return Matrix::Identity() -
           Scalar(0.5) * V +
           (Scalar(1)/theta2 - (Scalar(1) + std::cos(theta))/(Scalar(2)*theta*std::sin(theta))) * (V * V);
}
...
template<typename _Scalar>
typename SO3<_Scalar>::AdjointMatrix
SO3<_Scalar>::ad(const TangentVector& v) {
...
#endif // SOFA_COMPONENT_COSSERAT_LIEGROUPS_SO3_INL
