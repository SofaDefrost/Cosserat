...
cosserat_config1 = {'init_pos': [0., 0., 0.], 'tot_length': TOTAL_LENGTH, 'nbSectionS': NB_SECTIONS,
                    'nbFramesF': NB_FRAMES}
cosserat_config2 = {'init_pos': [0., 1., 0.], 'tot_length': TOTAL_LENGTH, 'nbSectionS': NB_SECTIONS,
                    'nbFramesF': NB_FRAMES}
...
class Animation(Sofa.Core.Controller):
    ...
    def onKeypressedEvent(self, event):
        ...
        if ord(key) == 18:  # left
            ...
            with self.rigidBaseMO.rest_position.writeable() as posA:
                self._extracted_from_onKeypressedEvent_17(posA) 

    # TODO Rename this here and in `onKeypressedEvent`
    def _extracted_from_onKeypressedEvent_17(self, posA):
        #QInit = Quat.createFromEuler([0.5, 0, 0], 'ryxz', inDegree=True)
        QInit = Quat.createFromAxisAngle([1., 0., 0.], pi/2.)
        QInit.normalize()
        qTemp=Quat()
...
def createScene(rootNode):
    ...
    rigidBaseNode2.addObject('MechanicalObject', name="rigidState", template='Rigid3d',
                                # position="2 4 1 0.7071 0 0.7071 0",
                                rest_position="2 0. 0 0 0 0 1",
                                showObject=True, showObjectScale=0.5)
    rigidBaseNode2.addObject('RestShapeSpringsForceField', name='rigid3', stiffness=1.e1, template="Rigid3d",
                                angularStiffness=1.e1, external_points=0, mstate="@rigidState", points=0)
    ...
    distanceNode.addObject('RigidDistanceMapping', input1=rigidBaseNode1.getLinkPath(),
                           input2=rigidBaseNode2.getLinkPath(),
